...
int main(int argc, char* argv[]) {
  ...
  OrtEnv* env = nullptr;
  CHECK_STATUS(
      g_ort->CreateEnv(ORT_LOGGING_LEVEL_WARNING, "test_sample_ep", &env));
  ...
  OrtStatus* status =
      g_ort->RegisterExecutionProviderLibrary(env, "SampleEP", plugin_path);
  if (status != nullptr) {
  ...
  if (status != nullptr) {
    std::cerr << "GetEpDevices failed: " << g_ort->GetErrorMessage(status)
              << std::endl;
    g_ort->ReleaseStatus(status);
    g_ort->ReleaseEnv(env);
    return 1;
  ...
  for (size_t i = 0; i < num_ep_devices; ++i) {
    ...
    std::cout << "    Name:           " << (ep_name ? ep_name : "(null)")
              << std::endl;
    std::cout << "    Vendor:         " << (ep_vendor ? ep_vendor : "(null)")
              << std::endl;
    std::cout << "    HW Device Type: " << type_str << std::endl;
    std::cout << "    HW Vendor:      " << (hw_vendor ? hw_vendor : "(null)")
              << std::endl;
    std::cout << "    HW Vendor ID:   0x" << std::hex << hw_vendor_id
              << std::dec << std::endl;
    std::cout << "    HW Device ID:   0x" << std::hex << hw_device_id
              << std::dec << std::endl;
    std::cout << std::endl;
  ...
  // =========================================================================
  std::cout << "Building test model with ops: Add, Sub, Mul, Div..."
            << std::endl;
  ...
  CHECK_STATUS(g_ort->SetTensorElementType(
      tensor_info, ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT));
  int64_t shape[] = {1, 4};
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    CHECK_STATUS(model_api->CreateNode(op_types[i], "", node_names[i], in_names,
        2, &out_name, 1, nullptr, 0, &node));
  ...
  CHECK_STATUS(g_ort->SessionOptionsAppendExecutionProvider_V2(
      session_options, env, &sample_ep_device, 1, nullptr, nullptr, 0));

  // Create session - this triggers GetCapability and shows which ops the EP
  // claims
  std::cout << "\nCreating session (EP will report claimed ops):" << std::endl;
  OrtSession* session = nullptr;
  status =
      model_api->CreateSessionFromModel(env, model, session_options, &session);

  g_ort->ReleaseModel(model);
  g_ort->ReleaseSessionOptions(session_options);

  if (status != nullptr) {
    std::cerr << "CreateSessionFromModel failed: "
              << g_ort->GetErrorMessage(status) << std::endl;
    g_ort->ReleaseStatus(status);
  } else {
  ...
  if (status != nullptr) {
    std::cerr << "UnregisterExecutionProviderLibrary failed: "
              << g_ort->GetErrorMessage(status) << std::endl;
    g_ort->ReleaseStatus(status);
    g_ort->ReleaseEnv(env);
    return 1;
