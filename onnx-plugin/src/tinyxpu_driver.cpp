...
#include "Varray.h"
#include "verilated.h"
// Required when Verilator is built with SystemC support (e.g. Homebrew on
// macOS)
...
TinyXPUDriver::TinyXPUDriver(const ApiPtrs& apis, std::string op_type_str,
    ...
    : ort_api(apis.ort_api),
      ...
      transB(transB_flag) {
  // NOTE: fused_relu, has_requant, requant_M0, requant_rshift, requant_zp, bias
  // removed.
...
OrtStatus* ORT_API_CALL TinyXPUDriver::ComputeImpl(OrtNodeComputeInfo* this_,
  ...
  // ---- fetch inputs -------------------------------------------------------
  // QLinearMatMul: A=input[0], B=input[3] (inputs 1,2,4,5,6,7 are scale
  // constants). MatMulInteger / MatMul / Gemm: A=input[0], B=input[1].
  const uint32_t b_idx = (info->op_type == "QLinearMatMul") ? 3u : 1u;

  const OrtValue* input_A = nullptr;
  const OrtValue* input_B = nullptr;
  OrtStatus* status =
      info->ort_api->KernelContext_GetInput(kernel_context, 0, &input_A);
  ...
  auto read_shape = [&](const OrtValue* t,
                        std::vector<int64_t>& dims) -> OrtStatus* {
    OrtTensorTypeAndShapeInfo* si = nullptr;
  ...
  // QLinearMatMul still reaches ComputeImpl but must be handled in software
  // after the Verilator array returns int32 results, or handled outside this
  // EP.
  ...
  // A_sl: [total_M, K_sl], B_sl: [K_sl, N_sl] (contiguous), C_sl: [total_M,
  // N_sl]. K_sl <= HW_ROWS and N_sl <= HW_COLS must hold. Requantization (if
  // needed) must happen in software after the array returns.
  auto run_slice = [&](const int8_t* A_sl, const int8_t* B_sl, int32_t* C_sl,
                       int8_t* /* C_i8_sl */, int64_t total_M, int64_t K_sl,
                       int64_t N_sl, SimObservations& obs) {
  ...
  // Tiles along K are summed into the int32 output; tiles along N are
  // independent. (if needed for QLinearMatMul) must happen in software after
  // the array returns.
