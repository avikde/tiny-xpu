...
const TinyXpuPerfCounters& tinyxpu_get_last_perf_ref() { return g_last_perf; }

TinyXpuPerfCounters TinyXpuPerfCounters::from_observations(
  ...
  c.mac_efficiency =
      (c.hw_mac_events > 0) ? (double)c.useful_mac_ops / c.hw_mac_events : 0.0;

  // ── Cycles ───────────────────────────────────────────────────────────────
  c.ticks_overhead = obs.ticks_total - obs.ticks_streaming;
  c.overhead_frac =
      (obs.ticks_total > 0) ? (double)c.ticks_overhead / obs.ticks_total : 0.0;
  ...
  c.ai_systolic = (c.total_mem_bytes > 0)
                      ? (double)c.useful_mac_ops / c.total_mem_bytes
                      : 0.0;
  ...
  c.ai_scalar = (c.scalar_mem_bytes > 0)
                    ? (double)c.useful_mac_ops / c.scalar_mem_bytes
                    : 0.0;
  ...
  c.e_total_pj =
      c.e_compute_pj + c.e_weight_pj + c.e_activation_pj + c.e_output_pj;
