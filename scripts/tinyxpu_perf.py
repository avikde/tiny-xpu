...
def print_perf(perf: _TinyXpuPerfCounters) -> None:
    ...
    print(
        f"  Overhead fraction   : {perf.overhead_frac * 100:.1f}%  (shrinks as M grows)"
    )

    print("  -- Compute " + "-" * 56)
    print(
        f"  Physical MACs       : {perf.hw_mac_events}"
        f"  ({o.ticks_streaming} ticks x {o.hw_rows}x{o.hw_cols} PEs)"
    )
    print(
        f"  Useful MACs         : {perf.useful_mac_ops}"
        f"  (M x K x N = {o.M}x{o.K}x{o.N})"
    )
    pipeline_lat = o.hw_rows + o.hw_cols - 2
    print(
        f"  MAC efficiency      : {perf.mac_efficiency * 100:.1f}%"
        f"  (= M/(M+pipeline_lat) = {o.M}/{o.M + pipeline_lat}, pipeline_lat={pipeline_lat})"
    )

    print("  -- Data movement " + "-" * 50)
    print(
        f"  Weight writes       : {o.weight_writes} elems x 1 B = {perf.weight_bytes} B  (SRAM, once)"
    )
    print(
        f"  Activation writes   : {o.activation_writes} elems x 1 B = {perf.activation_bytes} B  (DRAM, streamed)"
    )
    print(
        f"  Output reads        : {o.output_reads} elems x 4 B = {perf.output_bytes} B  (DRAM, written)"
    )
    print(
        f"  Total               : {perf.total_mem_bytes} B"
        f"  ->  arith. intensity {perf.ai_systolic:.2f} MAC/B"
    )
    print(
        f"  Scalar no-cache     : {perf.scalar_mem_bytes} B"
        f"  ->  arith. intensity {perf.ai_scalar:.2f} MAC/B"
    )
    print(
        f"  Weight reuse factor : {o.M}x"
    ...
    print(
        f"  Ridge-point BW      : {ridge_bw:.1f} B/cycle  (need this DRAM BW to be compute-bound)"
    )
    for label, bw in [
        ("4 B/cyc (narrow)", 4),
        ("16 B/cyc (typical)", 16),
        ...
        print(
            f"    {label:22s}: {bound}  (peak perf = {min(peak_macs_cycle, bw * perf.ai_systolic):.1f} MACs/cycle)"
        )
    ...
    print(
        f"  Compute (phys MACs) : {perf.e_compute_pj:7.1f} pJ  ({ratio:.1f}x more than useful-only)"
    )
